...
public class NinjaInventory {
    ...
    public void purchaseItem(com.github.hirotask.mc1171.Ninja ninja, ItemStack item) {
        if(ninja.getMoney() > 0) {
            ...
            if (inv.contains(com.github.hirotask.mc1171.inventory.ItemManager.getMoney().getType())) {
                java.util.HashMap<Integer, ? extends ItemStack> indexs = inv.all(com.github.hirotask.mc1171.inventory.ItemManager.getMoney().getType());
                for (int key : indexs.keySet()) {
                    if (key == 18) {
                        int amount = inv.getItem(key).getAmount();
                        if (amount > 1) {
